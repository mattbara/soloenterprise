...
export type WorkerType = 'backend' | 'echo' | 'orchestrator' | 'frontend' | 'qa' | 'devops' | 'feedback';
...
export async function getAllWorkerStatuses(): Promise<Record<WorkerType, WorkerStatus>> {
  const types: WorkerType[] = ['backend', 'echo', 'orchestrator', 'frontend', 'qa', 'devops', 'feedback'];
